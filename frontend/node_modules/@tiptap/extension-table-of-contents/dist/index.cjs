...
function generateTocIds(doc, extensions, extensionName = "tableOfContents") {
  ...
  const schema = (0, import_core2.getSchema)([...extensions.filter((ext) => ext.name !== extensionName), tocExtension]);
  const anchorTypes = (_a = tocExtension.options.anchorTypes) != null ? _a : ["heading"];
  const getId = (_b = tocExtension.options.getId) != null ? _b : (() => (0, import_uuid3.v4)());
  const contentNode = import_model.Node.fromJSON(schema, doc);
  const updates = [];
  const existingIds = [];
  ...
  const tr = import_state2.EditorState.create({ doc: contentNode }).tr;
